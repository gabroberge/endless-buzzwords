#!/usr/bin/env bash
# Configure GitHub repository security settings for endless-buzzwords.
# Requires: gh auth login (repo scope)
set -euo pipefail

REPO="${1:-gabroberge/endless-buzzwords}"
BRANCH="${2:-master}"
CI_CHECK="Build"
...
gh api --method PUT "repos/${REPO}/topics" \
  --input - <<'EOF'
{
  "names": ["satire", "astro", "buzzwords", "content-ops", "static-site", "technical-brand"]
}
EOF
